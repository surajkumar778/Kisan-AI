...
exports.getProfile = async (req, res) => {
  try {
    const farmer = await Farmer.findOne({ googleId: req.user.googleId })
...
exports.upsertProfile = async (req, res) => {
  try {
    const { name, email, googleId, avatar, phone, location, farm, language } = req.body
    const farmer = await Farmer.findOneAndUpdate(
      { googleId },
      { $set: { name, email, googleId, avatar, phone, location, farm, language, lastLogin: new Date() } },
...
exports.deleteAccount = async (req, res) => {
  try {
    await Farmer.findOneAndDelete({ googleId: req.user.googleId })
